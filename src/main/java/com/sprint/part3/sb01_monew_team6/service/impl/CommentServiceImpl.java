...
public class CommentServiceImpl implements CommentService {
  ...
  public CommentDto register(CommentRegisterRequest request) {
    ...
    NewsArticle article = newsArticleRepository.findById(request.articleId())
        .orElseThrow(
            () -> new NewsException(ErrorCode.NEWS_ARTICLE_NOT_FOUND_EXCEPTION, Instant.now(),
                HttpStatus.NOT_FOUND));

    //  2. User 조회
    User user = userRepository.findById(request.userId())
        .orElseThrow(
            () -> new UserException(ErrorCode.USER_NOT_FOUND, Instant.now(), HttpStatus.NOT_FOUND));
  ...
  public List<CommentDto> getComments(Long articleId) {
    ...
    if (!newsArticleRepository.existsById(articleId)) {
      throw new NewsException(ErrorCode.NEWS_ARTICLE_NOT_FOUND_EXCEPTION, Instant.now(),
          HttpStatus.NOT_FOUND);
    }
